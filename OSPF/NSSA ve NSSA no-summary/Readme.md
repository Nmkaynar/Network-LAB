...
``router ospf <process id >`` altında ``area <area id> nssa`` şeklinde yapılandırılır
 ...
 Topoloji de R1 Router'ı ``192.168.1.0/24`` networkünü external olarak duyuracak.  Bu işlem sonrası R3 ve R4 üzerinde ``show ip ospf database`` ve ``show ip route ospf | begin Gate``  çıktısı aşağıdaki gibidir. ASBR router olan R1 ``192.168.1.0/24`` networkünü LSA 5 ile duyurur ve R3/R4 routerların routing tablelarında ``O E2`` şeklinde gözükür.
...
Bu confige ``area 1 nssa default-information-originate`` komutunuda dahil edersek,

ABR router(R2) hem LSA 3 olarak  hem de LSA 7 olarak default rotayı area 1'e enjekte eder.

<img width="723" height="609" alt="image" src="https://github.com/user-attachments/assets/3652bcca-dd3e-486e-9d11-24ba45dd2117" />

Routing table batığımızda LSA 3 olan tercih edildiğini görüyoruz. Bunun sebebi, OSPF'in best path type seçiminde vermiş olduğu önceliklendirmeden kaynaklanmaktadır.
Aynı rota için  intra-area (O) > Inter-area (O IA) > Type 1 External (E1/N1) > Type 2 External (E2/N2) şeklindedir.
